...
//   classCount: z.number().optional().refine(val => val !== undefined, { message: "Class count is required for presencial courses" }), // Optional for presencial courses
//   mode: z.boolean().default(false), // false = presencial, true = online
//   classSchedule: z.array(z.string()).optional().refine(val => val && val.length > 0, { message: "Class schedule is required for presencial courses" }), // Only for presencial courses
// }).refine((data) => {
